...
class FCNs(nn.Module):
    ...
    def forward(self, x):
        ...
        score = self.bn1(self.relu(
            self.deconv1(x5)))  # size=(N, 512, x.H/16, x.W/16)
        score = score + x4  # element-wise add, size=(N, 512, x.H/16, x.W/16)
        score = self.bn2(self.relu(
            self.deconv2(score)))  # size=(N, 256, x.H/8, x.W/8)
...
class VGGNet(VGG):
    def __init__(self,
        ...
        if remove_fc:  # delete redundant fully-connected layer params
            del self.classifier
